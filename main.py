...
FILENAME = 'short.csv'
OUTPUT_FILENAME_FULL = 'data_full.csv'
OUTPUT_FILENAME = 'data.arff'
GRAPHIC_FILES = 'jpg|gif|bmp|xbm|png|jpeg|mpg|mpeg'
day_hour_separated = False
create_arff_file = False
...
for line in tqdm(lines):
    ...
    method = head_parts[0]
    address = head_parts[1].replace(',', '').replace(' ', '').replace('%', '').replace('+', '').lower()
    ...
    if day_hour_separated:
        row = [ip, day, hour, method, address, protocol, status, bytes]
    else:
        row = [ip, date, method, address, protocol, status, bytes]

    log.append(row)

if day_hour_separated:
    data = pd.DataFrame(data=log, columns=['ip', 'day', 'hour', 'method', 'address', 'protocol', 'status', 'bytes'])
else:
    data = pd.DataFrame(data=log, columns=['ip', 'date', 'method', 'address', 'protocol', 'status', 'bytes'])

data = data[data.status == 200]
data = data[~data.address.str.contains(GRAPHIC_FILES)]

data.to_csv(OUTPUT_FILENAME_FULL, index=False, header=True)
